#!/usr/bin/env python3
"""
Ultra-comprehensive Unified CSV Analyzer CLI
Standalone script for deep analysis of unified trading data
"""
...
def find_latest_unified_csv(csv_dir: Path = None):
    ...
    if csv_dir is None:
        csv_dir = Config.OUTPUT_DIR / "unified_csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Ultra-comprehensive analysis of unified trading CSV files"
    )
    ...
    parser.add_argument(
        "--charts-only", 
        action="store_true",
        help="Generate charts only, skip report"
    )
    
    parser.add_argument(
        "--holdings-only",
        action="store_true",
        help="Analyze holdings only (fastest)"
    )
    ...
    if args.csv_file:
        ...
        if not csv_file:
            ...
            print("💡 Run: python3 main.py --unified-csv")
            return 1
    ...
    try:
        ...
        analyzer = UnifiedCSVAnalyzer(
            str(csv_file),
            str(fund_mapping_file) if fund_mapping_file else None
        )
        ...
        if args.holdings_only:
            ...
            if not holdings.empty:
                ...
                for _, holding in top5.iterrows():
                    print(f"{holding['symbol']}: ¥{holding['total_cost_jpy']:,.0f} ({holding['portfolio_weight']*100:.1f}%)")
            else:
            ...
            if report['asset_allocation']['by_asset_class']:
                print(f"\n🏷️  Asset Allocation:")
                for asset_class, pct in sorted(report['asset_allocation']['by_asset_class'].items(),
            ...
            if report['top_holdings']:
                ...
                for i, holding in enumerate(report['top_holdings'][:5], 1):
                    print(f"  {i}. {holding['symbol']} - ¥{holding['total_cost_jpy']:,.0f} ({holding['portfolio_weight']*100:.1f}%)")
