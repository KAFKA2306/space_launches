...
class TradeVisualizer:
    ...
    def plot_portfolio_overview(self, holdings_df: pd.DataFrame, summary: Dict, output_path: Path):
        ...
        if not valid_holdings.empty:
            ax1.pie(
                valid_holdings["current_value"],
                labels=valid_holdings["security_code"],
                autopct="%1.1f%%",
                startangle=90,
            )
        ...
        if "total_cost" in holdings_df.columns:
            ...
            if not cost_value_data.empty and cost_value_data["total_cost"].max() > 0:
                ax3.scatter(
                    cost_value_data["total_cost"],
                    cost_value_data["current_value"],
                    alpha=0.6,
                )
