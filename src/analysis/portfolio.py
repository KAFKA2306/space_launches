...
class PortfolioAnalyzer:
        ...
        for _, trade in trades_df.iterrows():
            ...
            if transaction_type == "buy":
                ...
                if holdings[security_code]["total_cost"] > 0:
                    avg_cost_per_share = holdings[security_code]["total_cost"] / (
                        holdings[security_code]["total_shares"] + quantity
                    )
    ...
    def _get_current_price(self, price_data: pd.DataFrame, security_code: str) -> float:
        ...
        try:
            # Try variations to match what's in price_data columns
            variations = [
                security_code,
                f"{security_code}.T" if str(security_code).isdigit() else security_code,
                str(security_code).replace(".T", ""),  # Try removing suffix if present
            ]

            for code_var in variations:
                if code_var in price_data.columns:
                    latest_price = price_data[code_var].iloc[-1]
