...
class UnifiedPortfolioAnalyzer:
    ...
    def unified_portfolio_analysis(self, trades_df: pd.DataFrame, 
    ...
    def _analyze_unified_holdings(self, trades_df: pd.DataFrame, 
                                price_data: Optional[pd.DataFrame]) -> pd.DataFrame:
        """統一ティッカーによる保有状況分析"""
        ...
        for ticker, data in holdings.items():
            if data['total_shares'] > 0:  # 現在保有中のもののみ
                ...
                holdings_data.append({
                    ...
                    'pnl_percentage': ((current_value_jpy - data['total_cost_jpy']) + data['realized_pnl_jpy']) / data['total_cost_jpy'] * 100 if data['total_cost_jpy'] > 0 else 0,
    ...
    def _analyze_by_currency_region(self, trades_df: pd.DataFrame) -> Dict:
        ...
        # 通貨別集計
        currency_summary = trades_df.groupby('currency').agg({
            'unified_amount': ['sum', 'count', 'mean'],
            'unified_ticker': 'nunique'
        }).round(2)
        currency_summary.columns = ['total_amount_jpy', 'transaction_count', 'avg_amount_jpy', 'unique_securities']
        
        # 地域別集計
        region_summary = trades_df.groupby('region').agg({
            'unified_amount': ['sum', 'count', 'mean'], 
            'unified_ticker': 'nunique'
        }).round(2)
        region_summary.columns = ['total_amount_jpy', 'transaction_count', 'avg_amount_jpy', 'unique_securities']
        
        # 通貨×地域クロス分析
        currency_region_cross = pd.crosstab(
            trades_df['currency'], trades_df['region'], 
            values=trades_df['unified_amount'], aggfunc='sum'
        ).fillna(0)
    ...
    def _analyze_by_sector_asset_class(self, trades_df: pd.DataFrame) -> Dict:
        ...
        # 資産クラス別集計
        asset_class_summary = trades_df.groupby('asset_class').agg({
            'unified_amount': ['sum', 'count'],
            'unified_ticker': 'nunique'
        }).round(2)
        
        # セクター別集計
        sector_summary = trades_df.groupby('sector').agg({
            'unified_amount': ['sum', 'count'],
            'unified_ticker': 'nunique'
        }).round(2)
    ...
    def _analyze_risk_return(self, trades_df: pd.DataFrame, 
    ...
    def _get_current_valuation(self, ticker: str, shares: float, 
                             price_data: Optional[pd.DataFrame]) -> Tuple[float, float]:
        """現在の評価額を取得"""
    ...
    def _calculate_diversification_score(self, trades_df: pd.DataFrame) -> float:
        ...
        diversity_score = min(100, (region_count + currency_count + asset_class_count) / max_possible_diversity * 100)
...
class CurrencyUnifier:
    ...
    def convert_to_base_currency(self, amount: float, from_currency: str, 
