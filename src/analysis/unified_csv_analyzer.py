...
class UnifiedCSVAnalyzer:
    ...
    def _load_unified_data(self) -> pd.DataFrame:
        ...
        numeric_cols = ['quantity', 'price', 'price_jpy_unified', 'settlement_amount', 
                       'amount_jpy_unified', 'conversion_rate']
    ...
    def analyze_current_holdings(self) -> pd.DataFrame:
        ...
        for symbol, holding in holdings.items():
            if holding['quantity'] > 0:  # Only current holdings
                avg_cost_per_unit = (holding['total_cost_jpy'] / holding['quantity'] 
    ...
    def calculate_performance_metrics(self) -> PerformanceMetrics:
        ...
        # Basic metrics
        total_return = (daily_values.iloc[-1] / daily_values.iloc[0] - 1) * 100
        days = len(daily_values)
        annualized_return = ((daily_values.iloc[-1] / daily_values.iloc[0]) ** (365.25 / days) - 1) * 100
    ...
    def _calculate_daily_portfolio_values(self) -> pd.Series:
        ...
        date_range = pd.date_range(start=cumulative_investment.index.min(),
    ...
    def analyze_trading_behavior(self) -> Dict:
        ...
        fund_trades = self.trades_df[self.trades_df['is_investment_fund'] == True]
        ...
        fund_ratio = len(self.trades_df[self.trades_df['is_investment_fund'] == True]) / len(self.trades_df)
    ...
    def generate_investment_recommendations(self) -> Dict:
        ...
        fund_holdings = self.holdings_df[self.holdings_df['is_fund'] == True]
    ...
    def generate_comprehensive_report(self, output_dir: str = None) -> Dict:
        ...
        report = {
            ...
            ].to_dict('records') if not holdings.empty else []
        }
    ...
    def _create_trading_timeline(self, output_path: Path):
        ...
        monthly_volume = self.trades_df.groupby(self.trades_df['trade_date'].dt.to_period('M'))['amount_jpy_unified'].sum()
