...
class CurrencyConverter:
    ...
    def __init__(self, config=None):
        ...
        self.currency_aliases = {
            ...
            'USD': ['USD', '米国ドル', '米ドル', 'USドル', 'アメリカドル', '$', 'dollar', 'us dollar'],
            'EUR': ['EUR', 'ユーロ', '欧州ユーロ', '€', 'euro', 'european euro'],
            'HKD': ['HKD', '香港ドル', 'HK$', 'hong kong dollar', 'hk dollar'],
            'HKドル': ['HKD', '香港ドル', 'HK$', 'hong kong dollar', 'hk dollar'],  # Special case
        ...
        self.fund_name_patterns = {
            ...
            # Geographic regions
            '全世界': ['全世界', 'world', 'global', 'all country', 'オール・カントリー'],
    ...
    def _is_investment_fund(self, security_name: str, security_code: str = "") -> bool:
        ...
        fund_indicators = [
            'ファンド', 'Fund', 'インデックス', 'Index',
            '投信', '投資信託', 'eMAXIS', 'iFree',
            'SBI', 'ＳＢＩ', '楽天', 'Rakuten',
            'ニッセイ', 'Nissay', 'Tracers'
        ]
    ...
    def add_unified_pricing_to_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Successfully added unified pricing columns")
            return df
