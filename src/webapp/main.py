...
def get_analyzer():
    ...
    if csv_path.exists():
        return UnifiedCSVAnalyzer(
            str(csv_path),
            str(fund_mapping) if fund_mapping.exists() else None
        )
    return None
    ...
    if analyzer:
        ...
        if not holdings.empty:
            ...
            summary = {
                ...
                "pnl_pct": (realized_pnl / total_value * 100) if total_value else 0,
            }
        ...
        if not df.empty:
            ...
            if q:
                mask = (
                    ...
                    df["security_name"].astype(str).str.contains(q, case=False, na=False)
                )
                df = df[mask]
...
async def refresh_data(request: Request):
    ...
    return templates.TemplateResponse("partials/refresh_result.html", {
        "request": request,
        "success": success,
